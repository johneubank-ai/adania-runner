...
import { mkdir, writeFile, stat, lstat, readlink, symlink, rm } from "node:fs/promises";
...
async function isClean(dir) {
  ...
  const r = await run("git", ["-C", dir, "status", "--porcelain"], { timeoutMs: 15_000 });
  return r.code === 0 && !r.out.trim();
}

// Point <agentDir>/kb at the org's SINGLE shared kb checkout (sharedKbDir) via a relative symlink, so every
// agent in an org shares ONE clone instead of N. Migrates a real per-agent clone left by the old model: it's
// replaced by the symlink only when it's a clean, fully-pushed git repo (nothing to lose); otherwise the
// standalone clone is kept and a warning is logged. Best-effort; never throws.
export async function linkAgentKb(agentDir, sharedKbDir, log = console.log) {
  const link = path.join(agentDir, "kb");
  const rel = path.relative(agentDir, sharedKbDir) || "."; // e.g. "../kb"
  const want = path.resolve(sharedKbDir);
  let st = null;
  try {
    st = await lstat(link);
  } catch {
  ...
  if (st?.isSymbolicLink()) {
    try {
      if (path.resolve(agentDir, await readlink(link)) === want) return; // already linked correctly
    } catch {
      /* dangling link → recreate */
    }
    await rm(link, { force: true }).catch(() => {});
  } else if (st) {
    // a real file/dir is in the way — only drop a clone we're certain is safe (clean + fully pushed).
    if (st.isDirectory() && (await isGitRepo(link)) && (await isClean(link)) && !(await hasUnpushed(link))) {
      await rm(link, { recursive: true, force: true }).catch(() => {});
      log(`  ↳ kb: ${agentDir}/kb was a redundant clone (clean + pushed) — replacing with a link to the shared kb`);
    } else {
      log(`  ↳ kb: ${agentDir}/kb has local/unpushed changes — keeping the standalone clone (not linking)`);
      return;
    }
  }
  await symlink(rel, link).catch((e) => log(`  ↳ kb: could not link ${agentDir}/kb -> ${rel}: ${e?.message ?? e}`));
}
