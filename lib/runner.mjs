...
async function runTurn(payload, bot, cwd, org) {
  ...
  try {
    ...
    // this bot's AgentDefinition slug AND the org's kb location. Without the kb owner the agent guessed it from
    // the connector name (mcp__adania-github__*) and 404'd on /repos/adania/kb; per-bot dirs put a synced ./kb
    // checkout in the agent's own cwd, so point it at LOCAL git (which also avoids the connector's 503s).
    const ghOrg = org?.githubOrgSlug;
    const ctxLines = [];
    if (cfg?.slug) ctxLines.push(`You are agent ${cfg.slug}.`);
    if (ghOrg)
      ctxLines.push(
        `Your knowledge base is ./kb in your working directory: a local git checkout of github.com/${ghOrg}/kb. ` +
          `Read and write it with local file + git operations on ./kb (pull to refresh, commit + push to save) ` +
          `rather than the GitHub connector. If you must use the connector for the kb, its owner is "${ghOrg}" ` +
          `and repo is "kb" (do NOT guess "adania").`,
      );
