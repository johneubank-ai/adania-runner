...
// Strict cap on how many turns EXECUTE at once across all agents. This is ONE machine running the full Claude
// Code toolset per turn, so unbounded parallelism thrashes CPU/memory/network. Turns beyond the cap are not
// dropped — they WAIT (FIFO) and start as running turns finish. Per-agent serialization still applies on top.
// Desktop-runner only; managed agents run server-side and are unaffected. Override via ADANIA_MAX_CONCURRENCY.
const MAX_CONCURRENCY = Number(process.env.ADANIA_MAX_CONCURRENCY ?? 4);
...
// A counting semaphore bounding concurrent turn execution. acquire() resolves immediately when a slot is free,
// otherwise waits FIFO until release() hands it a slot. release() transfers the slot straight to the next
// waiter (no decrement+re-increment), so the in-flight count can never exceed `max` under interleaving.
function makeSemaphore(max) {
  ...
  return {
    ...
    release: () => {
      ...
      if (next) next(); // hand this freed slot directly to the next waiter; active stays the same
      else active--;
    },
...
export async function runForever(getToken) {
  ...
  // Global cap on concurrent turn EXECUTION (across all agents). Persists across reconnects. Turns past the cap
  // wait here and are pulled as slots free — see the turn handler. Per-agent serialization (turnQueues) is the
  // separate "same agent never overlaps" guarantee; this is the "at most N total" guarantee on top.
  const turnSem = makeSemaphore(MAX_CONCURRENCY);
      ...
      if (f.type === "event" && f.turnId) {
        ...
        enqueueTurn(turnKey, async () => {
          try {
            ...
            await awaitProvisioning(orgId); // let this org's shared kb finish initializing before we run in it
            const org = orgId ? (orgsById.get(orgId) ?? null) : null; // carries githubOrgSlug for the kb context
            // Wait for a global execution slot (max MAX_CONCURRENCY at once); excess turns queue here, none drop.
            if (turnSem.stats().active >= MAX_CONCURRENCY) {
              console.log(`  ⏳ at capacity (${MAX_CONCURRENCY} running) — queued: ${label}`);
            }
            await turnSem.acquire();
            let reply;
            try {
              reply = await runTurn(payload, bot, cwd, org);
            } finally {
              turnSem.release(); // free the slot for the next queued turn, even if runTurn threw
            }
